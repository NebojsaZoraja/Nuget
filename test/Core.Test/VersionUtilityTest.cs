...
namespace NuGet.Test
{
    ...
    {
        [Theory]
        [InlineData("[net40]\\foo.dll", "4.0", ".NETFramework")]
        [InlineData("[sl35]\\sub\\bar.dll", "3.5", "Silverlight")]
        [InlineData("[winrt45]\\sub\\con.dll", "4.5", ".NETCore")]
        public void ParseFrameworkFolderNameRecognizeBrackets(string path, string expectedVersion, string expectedIdentifier)
        {
            // Act
            var frameworkName = VersionUtility.ParseFrameworkFolderName(path);
